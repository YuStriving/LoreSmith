...
def _save_test_outline(store: Store, outline_dict: dict) -> None:
    """把旧版 dict 形式的大纲转换成 OutlineEntry 列表后保存。"""
    entries: list[OutlineEntry] = []
    for ch in outline_dict.get("chapters", []) or []:
        entries.append(
            OutlineEntry(
                chapter=int(ch.get("chapter", 0) or 0),
                title=str(ch.get("title", "") or ""),
                # 旧测试里写的是 summary，新 API 字段名是 core_event
                core_event=str(ch.get("summary", "") or ch.get("core_event", "") or ""),
                hook=str(ch.get("hook", "") or ""),
                scenes=[str(s) for s in (ch.get("scenes", []) or [])],
            )
        )
    store.outline.save_outline(entries)
...
def test_single_chapter(output_dir: str) -> int:
    ...
    # 初始化基础数据
    store.outline.save_premise("测试小说前提：一个年轻人在雨夜觉醒，获得特殊能力。")
    _save_test_outline(store, {
        "chapters": [
            {
                "chapter": 1,
                "title": "雨夜觉醒",
    ...
    # 检查提交
    committed = store.drafts.load_chapter_text(1)
    if committed:
        print(f"✓ 章节已提交")
    else:
        print("✗ 章节未提交")
...
def test_multi_chapters(output_dir: str, num_chapters: int = 3) -> int:
    ...
    # 初始化基础数据
    store.outline.save_premise("测试小说前提：一个年轻人在雨夜觉醒，获得特殊能力，开始探索这个世界。")
    ...
    _save_test_outline(store, outline)
    ...
    success_count = 0
    for i in range(1, num_chapters + 1):
        chapter = store.drafts.load_chapter_text(i)
        if chapter:
...
def test_resume_flow(output_dir: str) -> int:
    ...
    # 初始化基础数据
    store.outline.save_premise("测试小说前提：一个年轻人在雨夜觉醒。")
    _save_test_outline(store, {
        "chapters": [
    ...
    # 检查第一章是否生成
    chapter1 = store.drafts.load_chapter_text(1)
    if not chapter1:
        print("✗ 第一章未生成，无法测试恢复")
        return 1
    ...
    # 检查第二章是否生成
    chapter2 = store.drafts.load_chapter_text(2)
    if chapter2:
...
def test_agent_components(output_dir: str) -> int:
    ...
    try:
        ...
        if isinstance(loop, LangGraphRuntime):
